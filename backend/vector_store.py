...
def _get_embeddings():
    """Embedding model aligned with LLM_PROVIDER."""
    ...
    if LLM_PROVIDER == "nvidia":
        if not NVIDIA_API_KEY:
            raise RuntimeError("NVIDIA_API_KEY not set in .env file.")
        return OpenAIEmbeddings(
    ...
    if LLM_PROVIDER == "openai_compatible":
        if not CUSTOM_LLM_BASE_URL:
            raise RuntimeError("CUSTOM_LLM_BASE_URL not set.")
        embed_model = os.getenv("CUSTOM_EMBED_MODEL", "text-embedding-3-small")
        return OpenAIEmbeddings(
    ...
    if LLM_PROVIDER == "google_genai":
        try:
            ...
            raise RuntimeError(
                "LLM_PROVIDER=google_genai requires langchain-google-genai for embeddings."
            ) from e
    ...
    raise RuntimeError(f"Unsupported LLM_PROVIDER for embeddings: {LLM_PROVIDER}")
...
class VectorStoreManager:
    def __init__(self):
        ...
        try:
            ...
            logger.info(f"VectorStoreManager initialized (LLM_PROVIDER={LLM_PROVIDER})")
